...
<br>After working on this longer today I have come to the conclusion that a GPT model is not the best approach for this system. Given more time I would've liked to train a new model from scratch rather than prompt engineer with gpt. However, I believe this system can be expanded on for helping future developers create new phishing detectors by understanding where the models fail better. Rather than training their models on even more data, the outputs can instead be told where to look in the email more and how to detect tricky things that these current models cant on their own.
